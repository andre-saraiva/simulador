...
plot "prob-normal.dat" using ($1):($2) title "DC 5.86%" with linespoints ls 1, \
"prob-normal.dat" using ($1):($2):($3) notitle with yerrorbars,\
"prob-normal.dat" using ($1):($4) title "DC 10.98%" with linespoints ls 2, \
"prob-normal.dat" using ($1):($4):($5) notitle with yerrorbars,\
"prob-normal.dat" using ($1):($6) title "DC 19.35%"  with linespoints ls 3, \
"prob-normal.dat" using ($1):($6):($7) notitle with yerrorbars,\
"prob-normal.dat" using ($1):($8) title "DC 42.85%"  with linespoints ls 4, \
